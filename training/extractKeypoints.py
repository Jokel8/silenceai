...
class HandKeypointsExtractor:
    ...
    def extract_hand_keypoints(self, image_path):
        try:
            # Bild laden
            image = cv2.imread(str(image_path))
            if image is None:
                print(f"Warnung: Bild {image_path} konnte nicht geladen werden")
                return None
                
            # BGR zu RGB konvertieren für Mediapipe
            rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            
            # Hand-Erkennung durchführen
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                for idx, (hand_landmarks, handedness) in enumerate(zip(results.multi_hand_landmarks, results.multi_handedness)):
                    # Bestimme ob es sich um linke oder rechte Hand handelt
                    ...
                    # Weise Keypoints zur entsprechenden Hand zu
                    if hand_label == 'Left':
    ...
    def process_subfolder(self, subfolder_path):
        ...
        for image_file in sorted(image_files):
            keypoints_data = self.extract_hand_keypoints(image_file)
